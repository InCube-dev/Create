package ru.vkprofi.hazmat.config;
...
public class SimpleConfig {
    ...
    private static final Logger log = LogManager.getLogger("[Hazmat] (config)");
    ...
    private SimpleConfig( ConfigRequest request ) {
        ...
        if( !request.file.exists() ) {
            ...
            try {
                ...
                log.error( identifier + " failed to generate!" );
                log.trace( e );
                broken = true;
            }
        }
        ...
        if( !broken ) {
            try {
                ...
                log.error( identifier + " failed to load!" );
                log.trace( e );
                broken = true;
            }
        }
